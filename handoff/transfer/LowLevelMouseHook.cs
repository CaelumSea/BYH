...
public sealed class LowLevelMouseHook : IMouseHook
{
        ...
        try
        {
            if (code >= 0)
            {
                ...
                if (messageType is { } resolved)
                {
                    ...
                    // R41: let subscribers veto (swallow) the event before it
                    // reaches the source app. Used by Ocean Eyes to intercept
                    // right-click for "redraw region" instead of letting the
                    // source app pop its context menu. SwallowCheck runs on the
                    // hook thread — subscribers must not block.
                    if (ShouldSwallow(eventData))
                    {
                        // Swallow: return non-zero so CallNextHookEx is skipped.
                        // The event is not raised to MouseEvent subscribers
                        // either (it's consumed entirely by the vetoing check).
                        return 1;
                    }
    ...
    /// <summary>
    /// R41: asks <see cref="SwallowCheck"/> subscribers whether this event
    /// should be swallowed (blocked from the source app). Any subscriber
    /// returning true wins. Each subscriber runs in isolation — a throw from
    /// one doesn't break the chain (treated as "no opinion" / false).
    /// </summary>
