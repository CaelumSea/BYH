...
public partial class App : Application
{
    ...
    public override void OnFrameworkInitializationCompleted()
    {
        if (ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            ...
            catch (ProviderConfigurationException)
            {
                ...
                _oceanEyesLoadWarning = "快捷键配置无效，已使用 Ctrl+Alt+Q 安全默认值。";
            }
            ...
            catch (ProviderConfigurationException)
            {
                ...
                _oceanEyesLoadWarning ??= "Ocean Eyes 截图配置无效，已使用默认值。";
            }
            ...
            catch (ProviderConfigurationException)
            {
                ...
                _spotlightLoadWarning = "Spotlight 快捷键配置无效，已使用 Ctrl+Alt+Space 安全默认值。";
            }
            ...
            catch (ProviderConfigurationException)
            {
                ...
                _spotlightLoadWarning ??= "工具栏快捷键配置无效，已使用 R/C/V 安全默认值。";
            }
        ...
        settingsWindow.PromptTemplateDeleted += OnPromptTemplateDeleted;
            ...
            _regionOverlay.RegionCancelled += () => { /* overlay hid itself */ };
            // R41: RegionReset is raised by overlay.Reset() (called from
            // _runtime.RegionResetRequested below). No direct subscriber here —
            // the runtime has already dismissed the toolbar + cleared OCR cache
            // before raising RegionResetRequested; we just need to tell the
            // overlay to clear its rect, which Reset() does inline.
            ...
            toolbarWindow.Opened += (_, _) =>
            {
                ...
                // R41: runtime right-click swallow → reset overlay for redraw.
                // The runtime has already hidden the toolbar + cleared OCR cache
                // (DismissOceanEyes); we tell the overlay to clear its rect +
                // re-arm UIA tracking so the user can immediately draw again on
                // the same overlay instance. The runtime raises this from the
                // hook thread; marshal to UI thread before touching the overlay.
    ...
    private void RegisterInitialOceanEyesHotKey()
    {
        ...
        try
        {
            ...
            _settingsWindow?.SetOceanEyesTriggerSettings(
                _oceanEyesTrigger,
                $"已注册：{_oceanEyesTrigger.ToDisplayText()}",
                isError: false);
        }
    ...
    private void RegisterInitialSpotlightHotKey()
    {
        ...
        try
        {
            ...
            _settingsWindow?.SetSpotlightTriggerSettings(
                _spotlightTriggerSettings,
                $"已注册：{_spotlightTriggerSettings.ToDisplayText()}",
                isError: false);
        }
        catch (Exception exception) when (exception is GlobalHotKeyRegistrationException or TimeoutException)
        {
    ...
    private void OnOceanEyesTriggerSettingsSaved(OceanEyesTriggerSettings requested)
    {
        ...
        string shortcutState = requested.KeyboardShortcutEnabled
            ? $"已保存：{requested.ToDisplayText()}"
            : "键盘快捷键已停用";
        string chordState = requested.MouseChordEnabled ? "左右键已开启" : "左右键已关闭";
        _settingsWindow?.SetOceanEyesTriggerSettings(
            requested,
    ...
    private void OnOceanEyesCaptureSettingsSaved(OceanEyesCaptureSettings requested)
    {
        ...
        _settingsWindow?.SetOceanEyesCaptureSettings(
            requested,
            "已保存：路径 " + requested.SavePath +
            (requested.AutoSaveEnabled ? " · 自动保存" : " · 不自动保存") +
            (requested.CopyToClipboardEnabled ? " · 复制" : " · 不复制") +
            (requested.UiaAssistEnabled ? " · 辅助框选" : " · 自由框选"),
            isError: false);
    }
    ...
    private void OnToolbarShortcutsSaved(ToolbarShortcutSettings requested)
    {
        ...
        _settingsWindow?.SetToolbarShortcuts(
            requested,
            "已保存：提示词 " + DisplayKey(requested.PromptKey) +
            " · 复制 " + DisplayKey(requested.CopyKey),
            isError: false);
    }

    /// <summary>Empty key = disabled, shown as "未绑定" in status lines.</summary>
    private static string DisplayKey(string? key) => string.IsNullOrEmpty(key) ? "未绑定" : key;
    ...
    private async Task RunOceanEyesCaptureAsync(int x, int y, int w, int h)
    {
        ...
        // Wait for the overlay's dim + rect + handles to be gone from the
        // compositor surface before BitBlt reads the screen.
        await WaitForCompositorSettleAsync().ConfigureAwait(true);

        // Capture the PNG bytes BEFORE showing the toolbar — otherwise the
        // toolbar itself would be in the shot. The bytes are cached in the
        // runtime so Enter can re-use them for save-to-disk without another
        // capture (and another self-capture risk).
        byte[]? png = ScreenRegionCapture.CaptureAsPng(x, y, w, h);
        if (png is null)
        {
            // Capture failed (zero-size region / BitBlt error). Drop silently —
            // the overlay already hid itself; nothing else to clean up.
            return;
        }
    ...
    {
        var openSettings = new NativeMenuItem("打开 BYH 设置");
        openSettings.Click += (_, _) => RefreshAndShowSettings();

        var openConfig = new NativeMenuItem("打开配置目录");
        ...
        var restart = new NativeMenuItem("重启 BYH");
        restart.Click += (_, _) => RequestRestart();

        var exit = new NativeMenuItem("退出 BYH");
