...
/// <summary>
/// Persistent input settings for opening the clipboard-history popup
/// (R54 <c>ClipboardHistoryWindow</c>). Mirrors
/// <see cref="SpotlightTriggerSettings"/> — keyboard-only, no mouse chord.
/// Default shortcut is <c>Ctrl+Alt+V</c> (avoids <c>Win+V</c>, matches BYH's
/// existing <c>Ctrl+Alt+Q/Space</c> family).
/// </summary>
public sealed record ClipboardHistoryTriggerSettings
{
    ...
    /// <summary>Main key (A-Z, 0-9, F1-F12, Space). Default V.</summary>
    public string Key { get; init; } = "V";

    public static ClipboardHistoryTriggerSettings Default { get; } = new();
    ...
    public ClipboardHistoryTriggerSettings Normalize() => this with
    {
