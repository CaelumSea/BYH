...
/// <summary>
/// AOT-safe, atomic persistence for Spotlight input triggers. Mirrors
/// <see cref="OceanEyesTriggerStore"/> but writes only the keyboard-shortcut
/// fields (no mouse-chord toggle — Spotlight is keyboard-only).
/// </summary>
public static class SpotlightTriggerStore
{
    public const int CurrentSchemaVersion = 1;
    public const int MaximumFileBytes = 8 * 1024;

    public static SpotlightTriggerSettings LoadIfExists(string path)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(path);
        if (!File.Exists(path))
        {
            return SpotlightTriggerSettings.Default;
        }
        ...
        try
        {
            ...
            {
                throw new ProviderConfigurationException("Spotlight 快捷键配置文件超过 8 KB 上限。");
            }
            ...
            {
                throw new ProviderConfigurationException("不支持的 Spotlight 快捷键配置 schemaVersion。");
            }

            SpotlightTriggerSettings defaults = SpotlightTriggerSettings.Default;
        ...
        {
            throw new ProviderConfigurationException("无法读取 Spotlight 快捷键配置文件。", exception);
        }
    }

    public static void Save(SpotlightTriggerSettings settings, string path)
    {
        ...
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException)
        {
            ...
            throw new ProviderConfigurationException("无法写入 Spotlight 快捷键配置文件。", exception);
        }
    }
