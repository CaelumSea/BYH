...
8. **Warp ownerless clipboard 仅限显式策略**。`warp.exe` 使用 `Ctrl+Shift+C`；Warp 可能写入无 Win32 owner HWND 的剪贴板，因此 `ClipboardCaptureInvocation.AllowOwnerlessResult` 只由 `CtrlShiftCOnly` 进程策略开启。仍必须满足目标窗口、剪贴板序号变化、稳定文本和读取期间序号未变化；其他应用不能走 ownerless 路径。
