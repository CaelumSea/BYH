...
- [DONE 第一百二十二批] Ocean Eyes Q 快捷键 OCR 文本提取弹窗 + 翻译弹窗源文本可编辑 + C 键复制关闭（新功能，非 audit 项）——Q 特判在 `DispatchToolbarActionKey` 最前（不入 PromptTemplate 系统，纯本地弹窗）；新 `OcrTextWindow`（可编辑 TextBox + C 加速器 + Esc 关闭）；翻译弹窗 SourceTextBox 改可编辑 + Retry 改带文本（`RetryWithTextRequested: Action<string>`）+ `TranslationSessionManager.RetryWithTextAsync`（重算方向 + 保留 action 上下文）；C 加速器焦点门控（`_textBoxFocused`/`_sourceTextBoxFocused` 标志，不用 FocusManager）；+8 i18n key；+6 测试（1 新 `PromptTemplateSetTests` 含 Q=null 契约 pin）；718/718 通过 0 回归；NativeAOT 0 trim/AOT 警告；exe +20KB；真机启动 OK。**真机验证四点修复（用户确认通过）**：① 划词按 C 复制后立刻关浮窗（补 `HideToolbarAndDisableHook()`）；② 弹窗默认查看模式不自动 focus 文本框（deferred `button.Focus()`）；③ 翻译弹窗源文本框去双框（`CardWarm` 外框 + `Bordered` 内框叠加 → 去内框）；④ 截图 OCR 隐藏浮窗避免被截（`NoActivateWindowHost.ShowAtCurrentPosition()` 新方法）。**待提交**。
- [DONE 第一百二十三批] 剪贴板历史滚动加载/增量渲染（性能优化，非 audit 项）——根因：`ScrollViewer`+普通 `ItemsControl`（Avalonia 12 无内置虚拟化面板）全量渲染上千条 row；`ReapplyFilter` 每次切标签/搜索/复制后重建 `_filteredRows` 全量 Add。方案：保留全量 `_allRows`（轻量），新增 `_filteredPool`（全量匹配集，仅计算）+ `_visibleCount`（已渲染条数），`_filteredRows` 只装首批 60 条；`ScrollChanged` 触底批量追加 40 条（`LoadMore`）；`MoveSelection` 跨越渲染边界时 `while` 循环 `LoadMore` 保证 ↓ 长按无缝；底部 `LoadMoreFooter` TextBlock 显示剩余条数。**搜索/标签语义零变化**（全量匹配集照常计算含归档，只是渲染层窗口化）。+1 i18n key × 3 文件（`Clip_LoadMore_Remaining`，`string.Format` + `{0}` 占位符沿用项目惯例）；718/718 通过 0 回归；NativeAOT 0 trim/AOT 警告；exe +6KB；真机启动 OK（131 entries，首批 60 + 底部提示剩余 71）。**未提交**。
- [DONE 第一百二十四批] Sensitive 改为仅手动添加（行为变更，非 audit 项）——用户要"sensitive 只有我主动添加"。删 `ClipboardClassifier.Classify` 的 Sensitive 分支（永远不返回 Sensitive）+ 删 `IsSensitive(string)` 静态助手（保留 `SensitivePattern` 正则备查，NativeAOT 裁剪零成本）；`BuildEntry` 显式 `IsSensitive = false`。**手动路径完全不变**：`SetGroupOverride`（Move to → 🔒 Sensitive）仍是唯一设 `IsSensitive=true` 的路径，加密门/掩码/Sensitive tab/dedup 携带/IsProtected 全不动。**存量不动**：旧 `isSensitive:true` 条目继续加密+掩码（schema v5 持久化，零迁移）。测试：删 3 个废弃 Classify 测试 + 加 1 个契约 pin（`Classify_SensitivePatterns_NotAutoClassified_AfterBatch124`，7 case 断言 `NotEqual(Sensitive)`）；`ClipboardHistoryStoreTests.Entry()` 助手加 `isSensitive` 参数，4 个加密测试显式传 true。716/716 通过（Core 574→572 净 -2）0 回归；NativeAOT 0 trim/AOT 警告；exe -4.5KB；真机启动 OK（132 entries，encrypted=True 保持）。
- [DONE 第一百二十五批] R49 截图画廊 UI 全面重做 + 跨窗口置顶贴图（非 audit 项，多轮真机迭代）——外壳全量 IvoryJade 化（Window 背景 token、`Border.GalleryCard` 样式、删冗余标题行/时间标签）；交互反转（单击展开/双击删除确认，220ms 延迟区分）；翻页导航（←/→ 键 + 屏幕箭头 + N/M 计数）；**预览改用 `Popup`**（独立顶层窗口，图片能拖出画廊窗口，缩放/平移内核原样保留）；**防闪**（透明 card + 入场动画 0.85→1.0 掩盖 Popup 创建白闪 + 图片 Opacity 时序消除左上角闪现）；圆角（Image.Clip RectangleGeometry 16px，跟随缩放）；右键菜单（复制/置顶/打开目录/删除）+ 右键误关/误展开修复（左/右键分流）；悬停 bug 修复（`_isContextMenuOpen` 标志位屏蔽菜单打开期间的 SetSelected）。**新增跨窗口置顶贴图**：画廊预览右键 + 剪贴板图片右键都能置顶为贴图，共用 `SelectionRuntime.PinGalleryScreenshot`（private→public，画廊+剪贴板共用，文件路径→PinnedScreenshotWindow）。Core.Tests 574/574 通过 0 回归；NativeAOT 0 trim/AOT 警告；exe +14.6KB；真机验证通过。
- [DONE 项目迁移] selection-assistant → MyProjects/BYH（重命名 + 移动到用户项目目录）。非代码改动，robocopy /MOVE（2.887GB/3786 文件）。移动前需 kill 运行中的 BYH.exe + dotnet MSBuild 服务进程（5个，持有 bin/obj 句柄导致"Device or resource busy"）。git 状态完整保留（16 改动 + 历史 + 280 跟踪文件）。无 remote/worktree，移动不影响推送。
