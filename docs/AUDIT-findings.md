...
- [DONE 第一百二十四批] Sensitive 改为仅手动添加（行为变更，非 audit 项）——用户要"sensitive 只有我主动添加"。删 `ClipboardClassifier.Classify` 的 Sensitive 分支（永远不返回 Sensitive）+ 删 `IsSensitive(string)` 静态助手（保留 `SensitivePattern` 正则备查，NativeAOT 裁剪零成本）；`BuildEntry` 显式 `IsSensitive = false`。**手动路径完全不变**：`SetGroupOverride`（Move to → 🔒 Sensitive）仍是唯一设 `IsSensitive=true` 的路径，加密门/掩码/Sensitive tab/dedup 携带/IsProtected 全不动。**存量不动**：旧 `isSensitive:true` 条目继续加密+掩码（schema v5 持久化，零迁移）。测试：删 3 个废弃 Classify 测试 + 加 1 个契约 pin（`Classify_SensitivePatterns_NotAutoClassified_AfterBatch124`，7 case 断言 `NotEqual(Sensitive)`）；`ClipboardHistoryStoreTests.Entry()` 助手加 `isSensitive` 参数，4 个加密测试显式传 true。716/716 通过（Core 574→572 净 -2）0 回归；NativeAOT 0 trim/AOT 警告；exe -4.5KB；真机启动 OK（132 entries，encrypted=True 保持）。**未提交**。
